  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] rounded-full"
        style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(212,168,75,0.12) 0%, transparent 70%)' }}
      />

      {/* Gold arc decorations (matching flyer aesthetic) */}
      <div className="absolute top-0 left-0 w-64 h-64 pointer-events-none">
        <svg viewBox="0 0 260 260" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M0 0 Q130 0 260 130" stroke="rgba(212,168,75,0.3)" strokeWidth="1.5" fill="none" />
          <path d="M0 0 Q90 0 180 90" stroke="rgba(212,168,75,0.15)" strokeWidth="1" fill="none" />
        </svg>
      </div>
      ...
      {/* Floating orbs */}
      <div
        className="absolute top-1/4 right-1/4 w-96 h-96 rounded-full opacity-5 pointer-events-none"
        style={{ background: 'radial-gradient(circle, #D4A84B 0%, transparent 70%)', animation: 'float 8s ease-in-out infinite' }}
      />
      <div
        className="absolute bottom-1/3 left-1/4 w-64 h-64 rounded-full opacity-5 pointer-events-none"
        style={{ background: 'radial-gradient(circle, #1C3258 0%, transparent 70%)', animation: 'float 10s ease-in-out infinite reverse' }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-20 text-center">
        ...
        <h1 className="animate-on-scroll font-display text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold text-white leading-[1.05] mb-6">
          ...
          <span className="text-transparent bg-clip-text" style={{
            backgroundImage: 'linear-gradient(135deg, #D4A84B 0%, #F0D080 50%, #C5943A 100%)'
          }}>
            nos Estados Unidos
          </span>
        </h1>
        ...
        {/* Subtitle */}
        <p className="animate-on-scroll max-w-2xl mx-auto text-white/60 text-lg md:text-xl leading-relaxed mb-12 font-light">
